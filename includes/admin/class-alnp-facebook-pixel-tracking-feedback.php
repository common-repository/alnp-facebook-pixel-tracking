...
if ( ! class_exists( 'ALNP_FB_Pixel_Tracking_Feedback' ) ) {
	class ALNP_FB_Pixel_Tracking_Feedback {
		 ...
		 // Get the years.
			...
			if ( $years > 1 ) {
			 /* translators: Number of years */
				return sprintf( __( '%s years', 'alnp-facebook-pixel-tracking' ), $years );
			} elseif ( $years > 0 ) {
				return __( 'a year', 'alnp-facebook-pixel-tracking' );
			}
			...
			if ( $weeks > 1 ) {
				...
				return sprintf( __( '%s weeks', 'alnp-facebook-pixel-tracking' ), $weeks );
			} elseif ( $weeks > 0 ) {
				return __( 'a week', 'alnp-facebook-pixel-tracking' );
			}
			...
			if ( $days > 1 ) {
				...
				return sprintf( __( '%s days', 'alnp-facebook-pixel-tracking' ), $days );
			} elseif ( $days > 0 ) {
				return __( 'a day', 'alnp-facebook-pixel-tracking' );
			}
			...
			if ( $hours > 1 ) {
				...
				return sprintf( __( '%s hours', 'alnp-facebook-pixel-tracking' ), $hours );
			} elseif ( $hours > 0 ) {
				return __( 'an hour', 'alnp-facebook-pixel-tracking' );
			}
			...
			if ( $minutes > 1 ) {
				...
				return sprintf( __( '%s minutes', 'alnp-facebook-pixel-tracking' ), $minutes );
			} elseif ( $minutes > 0 ) {
				return __( 'a minute', 'alnp-facebook-pixel-tracking' );
			}
			...
			if ( $seconds > 1 ) {
				...
				return sprintf( __( '%s seconds', 'alnp-facebook-pixel-tracking' ), $seconds );
			} elseif ( $seconds > 0 ) {
				return __( 'a second', 'alnp-facebook-pixel-tracking' );
			}
		}
			...
			<div class="notice updated alnp-feedback-notice">
				<div class="alnp-feedback-notice-inner">
					<div class="alnp-feedback-notice-icon">
						...
						<img src="https://ps.w.org/alnp-facebook-pixel-tracking/assets/icon-256x256.png" alt="<?php printf( esc_attr__( '%1$s WordPress Plugin', 'alnp-facebook-pixel-tracking' ), esc_attr( $this->name ) ); ?>" />
					</div>
					<div class="alnp-feedback-notice-content">
						<?php /* translators: 1. Name */ ?>
						<h3><?php printf( esc_html__( 'Are you enjoying %1$s?', 'alnp-facebook-pixel-tracking' ), esc_html( $this->name ) ); ?></h3>
						<p>
							<?php /* translators: 1. Name, 2. Time */ ?>
							<?php printf( esc_html__( 'You have been using %1$s for %2$s now! Mind leaving a quick review to let me know know what you think? I\'d really appreciate it!', 'alnp-facebook-pixel-tracking' ), esc_html( $this->name ), esc_html( $time ) ); ?>
						</p>
					</div>
					<div class="alnp-install-now">
						<?php printf( '<a href="%1$s" class="button button-primary alnp-install-button" target="_blank">%2$s</a>', esc_url( ALNP_FB_PIXEL_REVIEW_URL . '#new-post' ), esc_html__( 'Leave a Review', 'alnp-facebook-pixel-tracking' ) ); ?>
						<a href="<?php echo esc_url( $no_bug_url ); ?>" class="no-thanks"><?php echo esc_html__( 'No thanks / I already have', 'alnp-facebook-pixel-tracking' ); ?></a>
					</div>
				</div>
			</div>
			<?php
...
/*
 * Instantiate the ALNP_FB_Pixel_Tracking_Feedback class.
 */
